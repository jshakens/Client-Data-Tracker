 ...
 * Contains a screen with a username and password.
 * A menu to switch between scenes.
 * A JavaFX application that displays 3 tables for tracking a clients Sleep and behaviors.
 * Each cell in the tables is editable. 
 */
...
public class CDT_2 extends Application  {
    ...
    File file = new File("C:\\Users\\jshap\\IdeaProjects\\CDT_2.0\\src\\sleep_data.txt");
    ...
    public void start(Stage primaryStage)throws IOException {
        ...
        uLabel.setFont(new Font("Arial",12));

        //label for password box
        Label p_label = new Label("Password");
        p_label.setLabelFor(passwordField);
        p_label.setFont(new Font("Arial",12));

        //Box for username and password screen
        VBox box1 = new VBox();
        box1.setAlignment(Pos.CENTER);
        box1.setSpacing(10);
        box1.getChildren().addAll(uLabel,username,p_label,passwordField, button);

        //Box for chrysalis logo
        HBox box2 = new HBox();
        ...
        ImageView imageA = new ImageView("Screenshot (7).png");
        imageA.setFitWidth(1300);
        imageA.setFitHeight(700);
        Menu viewMenu2 = new Menu("View");
        MenuItem Data = new MenuItem("Data");

        Data.setOnAction(event -> {
            primaryStage.setScene(scene);
        });
        menuBar2.getMenus().add(viewMenu2);
        MenuItem logout = new MenuItem("Logout");
        logout.setOnAction(event -> {
            primaryStage.setScene(mainScene);
        });
        viewMenu2.getItems().addAll(Data,logout);
        VBox BoxforArnie = new VBox();
        BoxforArnie.setAlignment(Pos.TOP_CENTER);
        BoxforArnie.getChildren().addAll(menuBar2,imageA);

        BorderPane supportPane = new BorderPane();
        supportPane.setLeft(BoxforArnie);
        Scene supportScene = new Scene(supportPane);


        Menu viewmenu = new Menu("View");
        MenuItem supportPlan = new MenuItem("Support Plan");
        viewmenu.getItems().addAll(supportPlan);
        supportPlan.setOnAction(event -> {
        ...
        vbox.getChildren().addAll(menuBar,label, table, s1, label2, table2, label3, table3);
        ...
        button.setOnAction(event -> {
                if (!username.getText().equals("jshakens") || !passwordField.getText().equals("123")) {
                    username.clear();
                    passwordField.clear();
                } else {
                    primaryStage.setScene(scene);
    ...
    /** public void sleepfile() throws IOException {
        try(readSleep){
            while(true){
                Sleep_Chart sleep = (Sleep_Chart) readSleep.readObject();
                String h1 = sleep.getH1();
            }
    ...
    public ObservableList<Sleep_Chart> getSleep() throws IOException {
